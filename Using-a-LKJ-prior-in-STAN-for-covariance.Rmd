...
## No Prior, Cholesky factor modified (MN)
...
```{r compilation3, eval=TRUE, cache=TRUE}
stanmodel3 <- stan_model(model_code = stancode3, model_name="stanmodel3")
samples3 <- sampling(stanmodel3, data = standata, init=inits, 
                    iter = 10000, warmup = 1000, chains = 4)
```

```{r}
library(coda) 
# codasamples3 <- do.call(mcmc.list, 
#   plyr::alply(rstan::extract(samples3, 
#                              pars=c("sigma", "Omega[1,2]", "Omega[1,3]", "Omega[2,3]"), 
#                              permuted=FALSE), 2, mcmc))
```
